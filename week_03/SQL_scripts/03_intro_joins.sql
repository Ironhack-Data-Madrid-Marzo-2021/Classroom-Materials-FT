...
/* What if we wanted to analyze how many units were sold for each title? 
We could declare our sales table first, our titles table second, and use a RIGHT JOIN to ensure that our query
returns a record for every title (even the ones that did not have any sales).*/
...
/*
The command for an outer join is typically FULL OUTER JOIN. However, MySQL does not support full outer joins, 
so we must improvise and perform them using a combination of the LEFT JOIN, RIGHT JOIN, and UNION commands. 
The left and right joins will collectively return all the records from each of the joined tables. 
The UNION command combines the results of two queries, and it should be placed directly between them. 
...
SELECT *
FROM publications.employee AS emp
RIGHT JOIN publications.jobs AS job
